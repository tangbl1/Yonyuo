...
import nc.itf.hi.psndoc.ITrainService;
import nc.itf.uap.IUAPQueryBS;
import nc.jdbc.framework.processor.BeanProcessor;
import nc.jdbc.framework.processor.ColumnProcessor;
import nc.jdbc.framework.processor.MapProcessor;
import nc.itf.hi.vo.TrainVO;
...
public class TrainServiceImpl implements ITrainService {
	...
	public String train(String xml)  {
		...
		try {
			...
			for(Element element:datalist){ 
				TrainVO trainVO = this.saveTrain(element);
				String isupdateSql = "select glbdef4,pk_psndoc_sub from hi_psndoc_train where glbdef14 = '"+trainVO.getGlbdef14()+"'";
				TrainVO resoultvo = (TrainVO) basedao.executeQuery(isupdateSql, new BeanProcessor(TrainVO.class));
				if(resoultvo==null){
	...
	private TrainVO saveTrain(Element element) throws BusinessException{
